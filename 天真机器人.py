...
TOTAL_MODELS = len(ALGORITHM_EXPRESSIONS)

def evaluate_expression(expr: str, history: List[Dict]) -> Dict:
    ...
    for offset in range(10):
        if offset >= len(history):
            break
        record = history[offset]
        nums = record.get('nums', [0, 0, 0])
        total = sum(nums)
        base = offset * 4
        n_values[base + 1] = nums[0] if len(nums) > 0 else 0
        n_values[base + 2] = nums[1] if len(nums) > 1 else 0
        n_values[base + 3] = nums[2] if len(nums) > 2 else 0
        n_values[base + 4] = total
    
    def replace_n(match):
        num = int(match.group(1))
        return str(n_values.get(num, 0))
...
def get_prediction(model_id: int, history: List[Dict]) -> Dict:
    if model_id < 1 or model_id > TOTAL_MODELS:
        return {'kill': '大单', 'size': '大', 'oe': '单', 'combo': '大单', 'value': 13}
    expr = ALGORITHM_EXPRESSIONS[model_id - 1]
    return evaluate_expression(expr, history)
